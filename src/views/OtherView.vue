  ...
  <div class="ImgView">
    <div>
      <div class="box">
        <div class="tops">
          <div class="tops-le">
            <el-button :icon="Download" class="download" style="font-size: 18px" @click="downImg">
              <div class="characters">下载</div>
            </el-button>
            <el-button :icon="DeleteFilled" class="delete" style="font-size: 18px" type="primary" size="small"
              @click="deleteImg">
              <div class="characters">删除</div>
            </el-button>
          </div>
          <div class="tops-rl">
            <el-input class="input" v-model="inputVal" size="large" placeholder="搜索你要的文件夹" :suffix-icon="Search"
              style="font-size: 16px" @keyup.enter="searchFile"></el-input>
          </div>
        </div>
        <div class="container">
          <div class="scroll-box">
            ...
            >
              <div @click="item.isChecked = !item.isChecked">
                <div class="img-box">
    ...
    (item: {
      ...
      if (item.name.indexOf(".") !== -1) {
        const allFileExtensions =
          /\.(jpeg|jpg|gif|png|bmp|ppt|pptx|doc|docx|text|xls|xlsx)$/i;
        if (!allFileExtensions.test(item.name.toString())) {
          picturePreview(item.path, token)
            .then((res) => {
              let compressedFileExtensions = /\.(zip|7z|rar)$/i;
              let videoFileExtensions =
                /\.(avi|wmv|mpeg|mpg|mov|rm|ram|swf|flv|mp4|mkv|webm|vob|ogv|ogg|drc|gifv|m4v|mts|m2ts|ts|qt|yuv|rmvb|asf|amv|m4p|m4v|mpg|mp2|mpeg|mpe|mpv|m2v|mxf|roq|nsv|f4v|f4p|f4a|f4b)$/i;

              if (compressedFileExtensions.test(item.name.toString())) {
                fileList.value.push({
                  img: require("../assets/zip.png"),
                  name: item.name,
                  isChecked: false,
                  path: item.path,
                });
              ...
              } else if (videoFileExtensions.test(item.name.toString())) {
                fileList.value.push({
